...
{
    public class When_LearningAimsReference_Import_File_Is_Valid_And_Db_Has_Existing_Non_Matching_Record : IClassFixture<LearningAimsReferenceTestFixture>, IDisposable
    {
        private readonly LearningAimsReferenceTestFixture _testFixture;
        private const string DataFilePath = @"LearningAimsReference\LearningAimsReference_Existing_Non_Matching_Record.csv";
        private int _createdRecordCount;
        private readonly string _testExecutionDirectory;

        public When_LearningAimsReference_Import_File_Is_Valid_And_Db_Has_Existing_Non_Matching_Record(LearningAimsReferenceTestFixture testFixture)
        {
            _testFixture = testFixture;
            _testExecutionDirectory = TestConfiguration.GetTestExecutionDirectory();
            _testFixture.ResetData();
            _testFixture.AddExisting("00100309", "Existing-LearningAimsReference");
        }
