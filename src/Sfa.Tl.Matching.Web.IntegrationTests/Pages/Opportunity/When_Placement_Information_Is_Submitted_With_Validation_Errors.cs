...
{
    public class When_Placement_Information_Is_Submitted_With_Validation_Errors : IClassFixture<CustomWebApplicationFactory<InMemoryStartup>>
    {
        private const int OpportunityItemId = 2000;

        private readonly CustomWebApplicationFactory<InMemoryStartup> _factory;

        public When_Placement_Information_Is_Submitted_With_Validation_Errors(CustomWebApplicationFactory<InMemoryStartup> factory)
        {
            _factory = factory;
        }
        ...
        [InlineData("tl-job-role", "JobRole", "ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVW", "You must enter a job role that is 100 characters or fewer", 1)]
        [InlineData("tl-job-role", "JobRole", "$%^&", "You must enter a job role using letters", 1)]
        [InlineData("tl-job-role", "JobRole", "35378$%&", "You must enter a job role using letters", 1)]
