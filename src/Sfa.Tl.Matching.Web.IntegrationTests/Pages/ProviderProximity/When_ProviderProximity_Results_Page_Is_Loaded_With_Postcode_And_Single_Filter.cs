...
{
    public class When_ProviderProximity_Results_Page_Is_Loaded_With_Postcode_And_Single_Filter : IClassFixture<CustomWebApplicationFactory<InMemoryStartup>>
    {
        private const string Title = "All providers in an area";

        private readonly CustomWebApplicationFactory<InMemoryStartup> _factory;

        public When_ProviderProximity_Results_Page_Is_Loaded_With_Postcode_And_Single_Filter(CustomWebApplicationFactory<InMemoryStartup> factory)
        {
            _factory = factory;
        }
