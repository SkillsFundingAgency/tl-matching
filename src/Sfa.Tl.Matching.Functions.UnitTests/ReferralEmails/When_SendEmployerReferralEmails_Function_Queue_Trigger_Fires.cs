...
namespace Sfa.Tl.Matching.Functions.UnitTests.ReferralEmails
{
    public class When_SendEmployerReferralEmails_Function_Queue_Trigger_Fires
    {
        ...
        )
        {
            ...
            //Act
            await employerReferralEmailFunctions.SendEmployerReferralEmailsAsync(data, context, logger, referralEmailService,
                functionLogRepo);

            //Assert
            await referralEmailService.Received(1).SendEmployerReferralEmailAsync(Arg.Any<int>(), Arg.Any<IList<int>>(), Arg.Any<int>(), Arg.Any<string>());
